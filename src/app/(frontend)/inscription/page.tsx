...
const INPUT_CLS =
  'border border-ink/20 rounded-lg px-4 py-2.5 w-full text-sm focus:border-[#1a7a3a] focus:outline-none'
  ...
  return (
    <div>
      <label htmlFor={id} className={LABEL_CLS}>
        {label}{required && <span className="text-[#1a7a3a]">*</span>}
      </label>
      <input
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F3] flex items-center justify-center py-16 px-4">
        <div className="max-w-lg w-full mx-auto bg-white rounded-2xl shadow-lg p-8 border border-ink/10 text-center">
          <CheckCircle2 size={52} className="mx-auto text-[#1a7a3a] mb-5" strokeWidth={1.5} />
          ...
          <Link
            href="/"
            className="mt-6 inline-block text-sm font-semibold text-[#1a7a3a] hover:text-ink transition-colors"
          >
            Retour à l&apos;accueil
          </Link>
        </div>
      ...
      <div className="max-w-lg w-full mx-auto bg-white rounded-2xl shadow-lg p-8 border border-ink/10">
        ...
        <div className="flex gap-1 mt-4">
          {STEPS.map((_, i) => (
            <div
              key={i}
              className={`h-1 flex-1 rounded-full ${i + 1 <= currentStep ? 'bg-[#1a7a3a]' : 'bg-ink/15'}`}
            />
          ))}
        </div>
        <div className="flex justify-between text-xs mt-2">
          {STEPS.map((label, i) => (
            <span key={label} className={i + 1 === currentStep ? 'text-[#1a7a3a] font-bold' : 'text-ink/30'}>
              {label}
            </span>
          ))}
        </div>
          ...
          <div className={currentStep === 1 ? 'flex flex-col gap-4' : 'hidden'}>
            ...
            <div>
              <span className={LABEL_CLS}>
                Genre<span className="text-[#1a7a3a]">*</span>
              </span>
            ...
            <div>
              <label htmlFor="motDePasse" className={LABEL_CLS}>
                Mot de passe<span className="text-[#1a7a3a]">*</span>
              </label>
            ...
            <button
              ...
              className="mt-2 w-full bg-[#1a7a3a] text-white py-3 rounded-lg font-semibold text-sm hover:bg-[#1a7a3a]/90 transition-colors"
            >
              Suivant →
            </button>
          </div>
          ...
          <div className={currentStep === 2 ? 'flex flex-col gap-4' : 'hidden'}>
            <div>
              <label htmlFor="fonctionProfessionnelle" className={LABEL_CLS}>
                Fonction<span className="text-[#1a7a3a]">*</span>
              </label>
              <select
            ...
            <div className="mt-2 flex justify-between gap-3">
              <button
                ...
                className="border border-ink/20 text-ink px-6 py-3 rounded-lg font-semibold text-sm hover:border-[#1a7a3a] hover:text-[#1a7a3a] transition-colors"
              >
                ← Retour
              </button>
              <button
                ...
                className="flex-1 bg-[#1a7a3a] text-white py-3 rounded-lg font-semibold text-sm hover:bg-[#1a7a3a]/90 transition-colors"
              >
                Suivant →
              </button>
            </div>
          ...
          <div className={currentStep === 3 ? 'flex flex-col gap-4' : 'hidden'}>
            <div>
              ...
              {photoPreview ? (
                ...
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-ink/20 px-4 py-5 text-sm text-ink/50 hover:border-[#1a7a3a] hover:text-[#1a7a3a] transition-colors"
                >
                  <Upload size={16} />
            ...
            <div>
              <label className={LABEL_CLS}>
                Pièce justificative<span className="text-[#1a7a3a]">*</span>
              </label>
            ...
            <div className="mt-2 flex justify-between gap-3">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 bg-[#1a7a3a] text-white py-3 rounded-lg font-semibold text-sm hover:bg-[#1a7a3a]/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading && (
        ...
        <p className="mt-6 text-center text-sm text-ink/60">
          Déjà membre ?{' '}
          <Link href="/connexion" className="text-[#1a7a3a] font-semibold">
            Se connecter
          </Link>
        </p>
      </div>
