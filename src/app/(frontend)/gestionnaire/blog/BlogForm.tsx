...
import { createBlogPostAction, updateBlogPostAction, uploadBlogMedia } from './actions'
...
export function BlogForm({ postId, initialValues }: Props) {
  ...
  async function handleSubmit(targetStatut: 'draft' | 'published') {
    ...
    try {
      ...
      if (imageFile) {
        ...
        const uploadResult = await uploadBlogMedia(fd)
