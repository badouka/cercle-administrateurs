...
import { Newspaper, PlusCircle, ArrowLeft } from 'lucide-react'
...
export default async function GestionnaireBlogPage() {
  ...
  const publiés    = posts.filter(p => p.statut === 'published').length
    ...
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8 space-y-8">
      ...
      {posts.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 px-6 py-12 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Créez votre premier article de blog pour commencer.</p>
          <Link
            href="/gestionnaire/blog/nouveau"
                ...
                {(posts as BlogPost[]).map(post => (
                  <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-black max-w-[280px] truncate">{post.titre}</p>
                      {post.slug && (
