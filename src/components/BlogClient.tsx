...
export function BlogClient({ posts }: { posts: BlogPostCard[] }) {
  ...
  const categories = useMemo(
    () =>
      [...new Set(posts.map(p => p.categorie?.trim()).filter((c): c is string => !!c))].sort(
  ...
  const filtered = useMemo(() => {
    ...
    return posts.filter(p => {
      if (filtre !== 'tous' && (p.categorie?.trim() ?? '') !== filtre) return false
      if (!q) return true
  ...
  return (
    <>
      <input
        ...
        className="mb-6 w-full rounded-xl border border-[#14110B]/15 bg-[#FAF8F3] px-5 py-3 text-sm text-[#14110B] placeholder:text-[#14110B]/40 focus:border-[#1a7a3a] focus:outline-none"
      />
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-7 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map(p => {
            ...
            const shareBtn =
              'inline-flex items-center gap-1.5 text-xs font-medium text-[#14110B]/60 border border-[#14110B]/15 px-3 py-1.5 rounded-lg hover:border-[#1a7a3a] hover:text-[#1a7a3a] transition-colors'
            return (
              <div
                key={p.id}
              ...
              >
                <Link href={`/blog/${p.slug || p.id}`} className="block cursor-pointer">
                  <div className="relative aspect-[16/10] overflow-hidden bg-[#EEF6F1]">
                    ...
                    {p.categorie && (
                      <span className="absolute left-3 top-3 rounded-full bg-white/90 px-3 py-1 text-xs font-bold text-[#1a7a3a] backdrop-blur-sm">
                        {p.categorie}
                      </span>
                    )}
                    {p.date && (
                  ...
                  <div className="px-5 pt-5">
                    ...
                    <span className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-[#1a7a3a]">
